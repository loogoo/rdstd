...
void dbLocalSemaphore::wait(rdMutex& mutex)
{ 
	while (count == 0) { 
...
bool dbLocalSemaphore::wait(rdMutex& mutex, time_t timeout) 
{
	if (count == 0) { 
...
void dbLocalSemaphore::signal(unsigned inc = 1) 
{
	count += inc;
	if (inc > 1) { 
...
void dbLocalSemaphore::open(unsigned initValue = 0) 
{ 
